...
const trajectories = TrajectoryService.getTrajectories('east', day);
...
async function main() {
    ...
    const resultDir = `./results/v${config.version}/east/aodv`;
    ...
    const { pathDurations } = ResultUtils.getPathDurations({ radioRange, trajectories, establishedPaths, simTime: 5 * hour  });

    ResultUtils.writeFile(`${resultDir}/summary-${hours}h-${day}.json`, {
        description: 'Eastbound, AODV',
        config,
