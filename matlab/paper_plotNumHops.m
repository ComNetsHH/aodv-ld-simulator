...
[pdrEastAodv_l, pdrEastAodv_m, pdrEastAodv_h] = getCI(getNumHops('east', 'aodv', version));
[pdrEastAodvLdD_l, pdrEastAodvLdD_m, pdrEastAodvLdD_h] = getCI(getNumHops('east', 'aodv-ld-d', version));
[pdrEastAodvLdS_l, pdrEastAodvLdS_m, pdrEastAodvLdS_h] = getCI(getNumHops('east', 'aodv-ld-s', version));
[pdrWestAodv_l, pdrWestAodv_m, pdrWestAodv_h] = getCI(getNumHops('west', 'aodv', version));
[pdrWestAodvLdD_l, pdrWestAodvLdD_m, pdrWestAodvLdD_h] = getCI(getNumHops('west', 'aodv-ld-d', version));
[pdrWestAodvLdS_l, pdrWestAodvLdS_m, pdrWestAodvLdS_h] = getCI(getNumHops('west', 'aodv-ld-s', version));
...
xticks([1,2,3])
xticklabels({'AODV', '\begin{tabular}{c} AODV-LD \\ (Deterministic) \end{tabular}', '\begin{tabular}{c} AODV-LD \\ (Stochastic) \end{tabular}'})
title('Average Number of Hops, Westbound', 'Interpreter','latex')
ylabel('\# Hops');
set(gca,'FontSize', font_size);
set(0, 'defaultTextInterpreter', 'latex');
set(gca, 'TickLabelInterpreter', 'latex');

saveas(fig,['../figures/num_hops_v' version '.png']);
saveas(fig,['../figures/num_hops_v' version '.fig']);
saveas(fig,['../figures/num_hops_v' version '.eps'], 'epsc');